...
model = dict(
    ...
    denoise_model=dict(
        ...
        dropout=0.0),
    diffusion_cfg=dict(
        betas=dict(type='linear', start=0.9, stop=0, num_timesteps=12),
        diff_iter=False,
        noise_components=dict(use_obj=True, use_bnd=True, use_unc=True)),
    test_cfg=dict(
        model_size=1024, fine_prob_thr=0.95, batch_max=32, iou_thr=0.15))
img_norm_cfg = dict(
...
train_pipeline = [
    ...
    dict(
        type='Collect',
        keys=['object_img', 'object_gt_masks', 'object_coarse_masks'])
]
        ...
        pipeline=[
            ...
            dict(
                ...
                keys=[
                    ...
                    'object_unc_map', 'object_edge_map'
                ])
        ],
        test_mode=False),
    val=dict(
