"""
Exp 6: Object + Uncertainty (No Boundary)
- Giống hệt Exp 8 nhưng tắt use_bnd
"""
_base_ = ['./segrefiner_oem_base.py']

# 1. Cấu hình thời gian huấn luyện (giống exp8)
max_iters = 120000
runner = dict(type='IterBasedRunner', max_iters=max_iters)

# 2. Cấu hình Learning Rate
lr_config = dict(
    policy='step',
    warmup='linear',
    warmup_iters=500,
...
work_dir = 'work_dirs/exp6_obj_unc'
...
data = dict(
    train=dict(pipeline=[
        ...
        dict(type='LoadOEMCoarseMasks', 
             use_obj=True, 
             use_bnd=False,  # ĐỒNG BỘ Ở PIPELINE
             use_unc=True, 
             test_mode=False),
