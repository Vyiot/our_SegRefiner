"""Exp 3: Chỉ Boundary-level noise"""
_base_ = ['./segrefiner_oem_base.py']
model = dict(diffusion_cfg=dict(noise_components=dict(use_obj=False, use_bnd=True, use_unc=False)))
data = dict(
    train=dict(pipeline=[
        dict(type='LoadImageFromFile'),
        dict(type='LoadAnnotations', with_bbox=False, with_label=False, with_mask=False, with_seg=True),
        dict(type='LoadOEMCoarseMasks', use_obj=False, use_bnd=True, use_unc=False, test_mode=False),
